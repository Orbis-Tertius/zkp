...
ProofSystem : ∀ {S : Set} → REL (S → Set) (S → ℕ → Set) _
ProofSystem D P = ∀ s → D s ⇔ Π[ P s ]


_[_⟺_] : ∀ {S : Set} → (_↔_ : Set → Set → Set) (P Q : S → Set) → Set
_↔_ [ P ⟺ Q ] = ∀ s → P s ↔ Q s

_⟺_ : ∀ {S : Set} (P Q : S → Set) → Set
P ⟺ Q = _⇔_ [ P ⟺ Q ]
...
--   -- zkp-soundness₁ : ¬ true (∀ λ n → get-prop n (prop-to-zkp false))
--   -- zkp-soundness₁ = ?
